...
| Sugestões de receitas — microfatia 5 | 🟡 | ✅ | `ai-adapter.js` envia ao proxy apenas `{query,maxResults}` após ação explícita. Os modelos locais curados cobrem intenções comuns e funcionam offline; IA generativa exige endpoint/proxy externo. Tempo e estilo são preferências de busca, não garantia nutricional. A busca limita texto a 200 caracteres, rejeita endpoint inseguro, normaliza respostas e volta para modelos locais identificados em falha. `app.js` nunca salva sugestão automaticamente; abre `form(null,prefill)` para revisão. |
